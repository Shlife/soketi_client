...
class SoketiClient {
  ...
  String? _socketId;
  final _eventData = StreamController<SoketiEvent>.broadcast();
  ...
  void _sendPing() {
    if (_singleton?._channel != null && (_singleton?._isConnected ?? false)) {
      ...
      debugPrint('Ping message sent');
    }
  }

  void _handlePong(dynamic data) {
    debugPrint('Pong answer reveived');
  }
  ...
  void connect() {
    ...
    debugPrint(_singleton?.host.toString());
  ...
  void _startPingTimer() {
    ...
    _singleton?._pingTimer = Timer.periodic(
        Duration(milliseconds: _singleton?.activityTimeout ?? activityTimeout),
        (timer) {
      _sendPing();
    });
  }
  ...
  void _eventHandler(event) {
    ...
    if (data is Map<String, dynamic>) {
      ...
      debugPrint(soketiEvent.eventName);
      if (soketiEvent.eventName == 'pusher:error') {
        ...
        debugPrint('Received ping event: ${soketiEvent.data}');
        _singleton?._channel?.sink
        ...
        while (_singleton?.channelToSubscribe.isNotEmpty ?? true) {
        ...
        debugPrint('${soketiEvent.eventName} - ${soketiEvent.data}');
        ...
        debugPrint('${soketiEvent.eventName} to ${soketiEvent.channelName}');
